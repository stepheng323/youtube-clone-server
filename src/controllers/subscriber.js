...
export const subscribeToChannel = catchAsync(async (req, res, next) => {
  ...
  const subscription = await Subscriber.findOne({ user: id, channel: channelId });
  if (subscription) return respondWithWarning(res, 409, 'Already subcribed');
  await Subscriber.create({
    channel: channelId,
    user: id,
  });
  return respondWithSuccess(res, 200, 'subcribed successfully');
});
...
export const unSubscribeFromChannel = catchAsync(async (req, res, next) => {
  ...
  return respondWithSuccess(res, 204, 'unsubcribed successfully');
});
...
export const getSubscriptionStatus = catchAsync(async(req, res, next) => {
  ...
  const { channel: { _id: channelId } } = videoDetails;
  const subscription = await Subscriber.findOne({ user: id, channel: channelId });
  if (!subscription) return respondWithWarning(res, 404, 'not subcribed to this channel');
  return respondWithSuccess(res, 200, 'you are subcribed to this channel', true);
});
...
export const subscriptionList = catchAsync(async (req, res, next) => {
  ...
  return respondWithSuccess(res, 200, 'subscription fetched successfully', paginatedResults);
});

export const channelSubscriptionCountVideo = catchAsync(async(req, res, next) => {
  const { videoId } = req.params;
  const videoDetails = await Video.findById(videoId).populate('channel');
  if (!videoDetails) return respondWithWarning(res, 404, 'no video with the id found');
  const { channel: { _id: channelId } } = videoDetails;
  const subscriptionCount = await Subscriber.countDocuments({ channel: channelId });
  return respondWithSuccess(res, 200, 'subscription count fetched successfully', subscriptionCount);
});
...
export const channelSubscriptionCount = catchAsync(async (req, res, next) => {
  ...
  return respondWithSuccess(res, 200, 'subcriber count fetched successfully', channelCount);
});
